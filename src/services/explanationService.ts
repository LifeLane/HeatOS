...
export const METRIC_KNOWLEDGE_BASE: Record<string, Partial<ExplanationMetadata>> = {
  ...
  ndvi: {
    ...
    unit: 'NDVI',
    dataType: 'MEASURED',
    source: 'Copernicus Sentinel-2 & FortyGuard Canopy Sensors',
    sourceInstitution: 'European Space Agency (ESA) & Microclimate Ground Array',
    spatialResolution: '10m Multispectral Satellite Mesh',
    whatItMeans: 'Normalized Difference Vegetation Index measuring vegetative health, chlorophyll activity, and photosynthetic canopy density on a scale from 0.0 to 1.0.',
    whyItMatters: 'Dense vegetative canopies provide vital shade and cooling through evapotranspiration, mitigating urban heat island retention and filtering air pollutants.',
    limitations: 'Satellite spectral reflectance can be temporarily influenced by cloud cover and seasonal leaf-drop cycles.',
    iconType: 'nature',
  },
  surfaceThermalMass: {
    metricId: 'surfaceThermalMass',
    label: 'Surface Thermal Mass',
    unit: '°C',
    dataType: 'MEASURED',
    source: 'FortyGuard Thermal Fabric',
    sourceInstitution: 'FortyGuard High-Density Urban Radiometric Array',
    spatialResolution: '1m - 10m Micro-Spatial Mesh',
    whatItMeans: 'Stored thermal energy and sensible heat retention capacity of dense urban hardscape materials (asphalt, concrete, stone, dark roofing).',
    whyItMatters: 'Surfaces with high thermal mass absorb intense solar irradiance during midday and continuously re-radiate infrared heat into the lower pedestrian boundary layer well into the night.',
    limitations: 'Thermal absorption varies based on surface albedo, material porosity, and localized shading.',
    iconType: 'heat',
  },
  locationsUnderWatch: {
    metricId: 'locationsUnderWatch',
    label: 'Locations Under Active Watch',
    unit: 'Sites',
    dataType: 'DERIVED',
    source: 'HeatOS Operational Monitoring',
    sourceInstitution: 'HeatOS Distributed Multi-Site Surveillance Engine',
    spatialResolution: 'Global / Municipal Multi-Asset Coordinates',
    whatItMeans: 'Total count of commercial sites, municipal zones, logistics hubs, and critical infrastructure assets currently monitored by HeatOS in continuous telemetry cycles.',
    whyItMatters: 'Provides operational oversight across distributed facilities to prioritize heat mitigation, workforce safety protocols, and cooling asset dispatch.',
    iconType: 'pulse',
  },
  activeHighRiskSignals: {
    metricId: 'activeHighRiskSignals',
    label: 'Active High-Risk Signals & Alarms',
    unit: 'Alarms',
    dataType: 'DERIVED',
    source: 'HeatOS Deterministic Event Engine',
    sourceInstitution: 'Verified Multi-Sensor Convergence Gateway',
    spatialResolution: 'Asset / Zone Specific Triggers',
    whatItMeans: 'Total number of active critical and elevated environmental alarms triggered by deterministic threshold crossings without statistical hallucination.',
    whyItMatters: 'Instantly surfaces acute operational hazards (extreme heat spikes, wet-bulb danger, air quality hazards) requiring immediate management intervention.',
    iconType: 'heat',
  },
  peakHeatDelta: {
    metricId: 'peakHeatDelta',
    label: 'Peak Urban Heat Island (UHI) Delta',
    unit: '°C',
    dataType: 'MEASURED',
    source: 'FortyGuard Microclimate Mesh',
    sourceInstitution: 'FortyGuard Thermal Gradient Engine',
    spatialResolution: '1m - 10m Micro-Spatial Differential',
    whatItMeans: 'The highest measured temperature differential between dense impervious urban zones and vegetated rural reference baselines across all monitored sites.',
    whyItMatters: 'Quantifies severe localized heat retention hotspots where built infrastructure exacerbates atmospheric temperatures above baseline synoptic forecasts.',
    limitations: 'Microclimate deltas fluctuate between shaded pedestrian alleys, open plazas, and asphalt corridors.',
    iconType: 'heat',
  },
  reportingNodes: {
    metricId: 'reportingNodes',
    label: 'Reporting Telemetry Nodes',
    unit: 'Nodes',
    dataType: 'MEASURED',
    source: 'FortyGuard IoT Mesh',
    sourceInstitution: 'FortyGuard High-Density Sensor Infrastructure',
    spatialResolution: 'Point Sensor Array (350 Target Capacity)',
    whatItMeans: 'Number of active IoT micro-weather stations, thermal radiometers, and air quality probes successfully transmitting synchronized telemetry in the current cycle.',
    whyItMatters: 'High node coverage (98%+) ensures spatial triangulation accuracy and eliminates data blind spots across complex urban terrain.',
    iconType: 'pulse',
  },
  telemetryLatency: {
    metricId: 'telemetryLatency',
    label: 'Telemetry Network Latency',
    unit: 'ms',
    dataType: 'MEASURED',
    source: 'HeatOS Realtime Stream Gateway',
    sourceInstitution: 'Low-Latency WebSocket & Edge Telemetry Pipeline',
    spatialResolution: 'System-Wide Transit Metric',
    whatItMeans: 'Round-trip transit and ingestion latency from physical sensor packet transmission to UI dashboard rendering.',
    whyItMatters: 'Sub-second precision (<50ms) ensures real-time operational response during acute environmental shifts and automated safety dispatches.',
    iconType: 'pulse',
  },
  throughput: {
    metricId: 'throughput',
    label: 'Encrypted Stream Throughput',
    unit: 'MB/s',
    dataType: 'MEASURED',
    source: 'HeatOS Telemetry Pipeline',
    sourceInstitution: 'TLS 1.3 End-to-End Encrypted Ingestion Buffer',
    spatialResolution: 'Aggregated Pipeline Bandwidth',
    whatItMeans: 'Continuous bi-directional data transfer rate across all active sensor channels, satellite feeds, and deterministic calculation pipelines.',
    whyItMatters: 'Guarantees uninterrupted high-frequency telemetry streaming without buffering, data truncation, or packet loss.',
    iconType: 'pulse',
  },
  lastPacket: {
    metricId: 'lastPacket',
    label: 'Latest Telemetry Packet Freshness',
    unit: 'Timestamp',
    dataType: 'MEASURED',
    source: 'FortyGuard High-Density Mesh',
    sourceInstitution: 'Continuous Real-Time Sync Stream',
    spatialResolution: 'Individual Sensor Packet Timestamp',
    whatItMeans: 'Timestamp and telemetry source of the most recent physical sensor data frame validated and ingested into the active state vector.',
    whyItMatters: 'Guarantees that environmental visualizations reflect real-time live conditions rather than cached historical baselines.',
    iconType: 'pulse',
  },
  dataQuality: {
    metricId: 'dataQuality',
    label: 'Data Quality & Stream Provenance',
    unit: 'Status',
    dataType: 'DERIVED',
    source: 'HeatOS State Engine',
    sourceInstitution: 'Multi-Source Convergence & Fallback Controller',
    spatialResolution: 'Per-Feed Status Check',
    whatItMeans: 'Surveillance status of upstream data feeds (NOAA, EPA, FortyGuard, Sentinel, USGS), verifying live synchronization and automated fallback caching.',
    whyItMatters: 'Guarantees 100% transparent non-fabrication of environmental metrics, explicitly flagging any cached or interpolated signals.',
    iconType: 'pulse',
  },
};
...
const METRIC_KEY_ALIASES: Record<string, string> = {
  ...
  canopydensity: 'ndvi',
  ndvi: 'ndvi',
  surface_thermal_mass: 'surfaceThermalMass',
  surfacethermalmass: 'surfaceThermalMass',
  thermal_mass: 'surfaceThermalMass',
  locations_under_watch: 'locationsUnderWatch',
  locationsunderwatch: 'locationsUnderWatch',
  total_watched: 'locationsUnderWatch',
  active_high_risk_signals: 'activeHighRiskSignals',
  activehighrisksignals: 'activeHighRiskSignals',
  high_risk_signals: 'activeHighRiskSignals',
  active_alerts: 'activeHighRiskSignals',
  alarms: 'activeHighRiskSignals',
  peak_heat_delta: 'peakHeatDelta',
  peakheatdelta: 'peakHeatDelta',
  uhi_delta: 'peakHeatDelta',
  uhidelta: 'peakHeatDelta',
  reporting_nodes: 'reportingNodes',
  reportingnodes: 'reportingNodes',
  telemetry_latency: 'telemetryLatency',
