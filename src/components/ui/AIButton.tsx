...
export const AIButton: React.FC<AIButtonProps> = ({
  children,
  icon = <Sparkles className="w-3.5 h-3.5 text-purple-600 flex-shrink-0" />,
  iconPosition = 'left',
  size = 'md',
  fullWidth = false,
  isLoading = false,
  ...
  const buttonContent = (
    <>
      {isLoading ? (
        <span className="inline-block w-4 h-4 border-2 border-purple-400 border-t-purple-700 rounded-full animate-spin flex-shrink-0" />
      ) : (
  ...
  const baseClasses = `inline-flex items-center justify-center font-semibold bg-purple-50 hover:bg-purple-100/90 active:bg-purple-200 text-purple-900 border border-purple-200/90 shadow-2xs hover:shadow-xs transition-all duration-150 focus-visible:ring-2 focus-visible:ring-purple-400 focus-visible:ring-offset-2 disabled:opacity-50 disabled:pointer-events-none cursor-pointer ${
    fullWidth ? 'w-full' : ''
