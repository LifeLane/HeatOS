...
export const EventCard: React.FC<EventCardProps> = ({
  ...
  return (
    <Card
      ...
      className={`p-4 sm:p-5 transition-all duration-200 hover:shadow-md flex flex-col justify-between ${
        isDismissed
    ...
    >
      <div>
        ...
        {/* ------------------------------------------------------------- */}
        <div className="p-3.5 rounded-2xl bg-slate-50 border border-slate-200/80 mb-3 space-y-2 text-xs leading-relaxed">
          {/* WHAT CHANGED */}
          <div>
        ...
        <div className="flex flex-wrap items-center gap-2 mb-3.5">
          {event.evidence.signals.map((sig, idx) => (
            <div
              key={idx}
              className="flex items-center gap-1.5 px-2.5 py-1 rounded-xl bg-white border border-slate-200/90 text-xs font-mono shadow-2xs"
            >
              <span className="text-slate-500 text-[11px]">{sig.metricName}:</span>
              <span className="font-bold text-slate-900">{sig.observedValue}</span>
        ...
        {/* ------------------------------------------------------------- */}
        <div className="p-3 rounded-2xl bg-gradient-to-r from-blue-50/60 to-indigo-50/40 border border-blue-100 flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-3">
          <div className="flex items-start gap-2.5">
            ...
            <div>
              <div className="text-[11px] font-bold text-blue-900 uppercase tracking-wider flex items-center gap-1.5">
                ...
                <span className="text-[9px] font-mono font-bold bg-blue-200/60 text-blue-800 px-1.5 py-0.2 rounded">
          ...
          <div className="flex items-center gap-1.5 text-[11px] text-slate-400 font-mono">
            ...
            {event.sources.map((src, i) => (
              <span
                ...
                className="px-1.5 py-0.5 rounded bg-slate-100 text-slate-600 font-semibold"
              >
                {src.sourceId}
              </span>
            ))}
        ...
        {/* ------------------------------------------------------------- */}
        {isEvidenceExpanded && (
          <div className="mb-4 pt-3 border-t border-slate-200/80 space-y-3 text-xs animate-fade-in">
            {/* Baseline Comparison */}
            ...
            {event.evidence.noiseRejectionRationale && (
              <div className="p-2.5 rounded-xl bg-emerald-50/60 border border-emerald-200/80 text-[11px] text-emerald-900 leading-relaxed">
      ...
      <div className="pt-3 border-t border-slate-100 flex flex-wrap items-center justify-between gap-1.5">
        <div className="flex flex-wrap items-center gap-1.5">
          ...
          <button
            ...
            className="flex items-center gap-1 px-2.5 py-1.5 text-xs font-bold rounded-lg bg-slate-100 hover:bg-slate-200 text-slate-700 transition-colors cursor-pointer"
          ...
          <button
            ...
            className="flex items-center gap-1 px-2.5 py-1.5 text-xs font-bold rounded-lg bg-blue-50 hover:bg-blue-100 text-[#2563EB] border border-blue-200/60 transition-colors cursor-pointer"
        ...
        {onDismiss && (
          <button
            ...
            className={`p-1.5 text-xs font-medium rounded-lg transition-colors cursor-pointer flex items-center gap-1 ${
              isDismissed
